...
export default function Dashboard() {
  ...
  const formatLiveTime = (emp) => {
    ...
    for (let i = emp.history.length - 1; i > 0; i--) {
      ...
      // If there was a gap of more than 40 seconds, they left and came back (40s prevents random resets from network jitter on 8s pings)
      const currTime = new Date(currentEvent.timestamp).getTime();
      const prevTime = new Date(prevEvent.timestamp).getTime();
      if (currTime - prevTime > 40000) break;
  ...
  const calculateDailyStats = (emp) => {
    ...
    for (let i = 1; i < todayPings.length; i++) {
        ...
        if (ping.room === currentRoom && (ping.time - lastPing <= 40000)) {
            lastPing = ping.time;
        } else {
